...
`dim_test` is the test metadata dimension. Export it from `domains/data/grid/test-grid.md` before the first run and update it when the scoped test list changes. A starter template is available at `domains/data/grid/dim_test_template.csv`.
...
| `lifecycle_stage` | When this test runs: Design, Development, CI, Pre-deployment, Deployment, or Production monitoring |
| `in_scope` | Boolean -- whether this test is active for the current project |

## How to build it

1. Export the test metadata from `domains/data/grid/test-grid.md` into a `dim_test` dimension using the schema above.
