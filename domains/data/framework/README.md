...
2. Map those artifacts to the dimensions in `domains/data/dimensions/`.
3. Use `domains/data/tests/` to identify applicable checks.
4. Use `domains/data/grid/README.md` to build a project-specific grid and owner matrix.
5. Use `domains/data/framework/dppf.md` to run a coverage self-assessment and identify gaps.
6. Use `domains/data/framework/maturity-model.md` to position the pipeline at its current maturity level.
7. Log and monitor every run using the guidance in `domains/data/dashboards/README.md`.
...
Tests in this framework are organized into eight domains drawn from DAMA-DMBOK, ISO/IEC 25012, the five pillars of data observability, and chaos data engineering practice. Each domain maps to one or more test category files in `domains/data/tests/`.

| Domain | Focus | Primary location |
|---|---|---|
| Structural | Schema, types, contracts, evolution | `domains/data/tests/schema-and-types.md` |
| Semantic | Business rules, referential integrity, reconciliation | `domains/data/tests/integrity-and-references.md` |
| Statistical | Distributions, anomalies, drift, volume | `domains/data/tests/anomaly-and-drift.md` |
| Temporal | Freshness, latency, ordering, late arrival | `domains/data/tests/performance-and-freshness.md` |
| Operational | Idempotency, retries, backfill, orchestration | `domains/data/tests/observability-and-operations.md` |
| Adversarial | Chaos, fault injection, poisoned input, replay | `domains/data/tests/adversarial.md` |
| Performance and scale | Throughput, degradation, cost, concurrency | `domains/data/tests/performance-and-freshness.md` |
| Sensibility | Outside-in plausibility, business context checks | `domains/data/tests/cross-validation-suite.md` |
