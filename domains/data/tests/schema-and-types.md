...
- Contract design review: before ingestion exists, confirm the proposed schema defines nullability and types up front, the contract carries a version scheme, and a reconciliation method is agreed for validating the pipeline once built.
...
| STR-016 | Data contract design review | Before ingestion exists, the proposed schema and data contract define nullability, types, and allowed values up front, carry a version scheme, and specify the reconciliation method that will validate the pipeline once built | Contracts finalized after the pipeline is already built with no agreed reconciliation method, ambiguous nullability or type expectations discovered mid-build, rework from a contract that was never validated as buildable before implementation started | Validity, Traceability | Design |
