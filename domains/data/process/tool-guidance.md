...
Start at `domains/data/README.md` for the full navigation map. The short path is:

1. `domains/data/framework/README.md` to understand the severity model and testing standard.
2. `domains/data/grid/test-grid.md` to see every test with its tier, threshold, owner, and DPPF ID.
3. `domains/data/process/test-cycle.md` to run a test cycle step by step.
4. `domains/data/tests/<category>.md` when you need to understand a specific test type in depth.

When starting a new project, use `domains/data/process/testing-strategy.md` to scope the engagement and decide which test IDs apply. When assessing an existing pipeline, open `domains/data/framework/dppf.md` and mark each ID as Covered, Partial, or Gap.
...
Each ID resolves to a test definition with the following fields, found in the DPPF catalog tables in `domains/data/tests/`:
...
| Lifecycle | Where in the pipeline lifecycle the test runs (Design, Development, CI, Pre-deployment, Deployment, Production monitoring) |

### How to select tests for a given pipeline

1. Read the attack surface map in `domains/data/framework/README.md` to identify which pipeline zones are in scope.
2. Map each zone to the relevant DPPF domains using the domain table in `domains/data/framework/README.md`.
3. For each in-scope domain, pull the catalog from `domains/data/tests/<matching file>.md`.
...
Load the checklist from `domains/data/framework/dppf.md`. It lists all 113 test IDs organized by domain. Any ID marked Gap or Partial is a coverage gap. Score gaps using the severity model in `domains/data/framework/README.md`:
...
`domains/data/grid/test-grid.md` is the master checklist. Each row has a DPPF IDs column listing the test catalog entries that the row covers. Use this to cross-reference grid rows with catalog entries, or to add new grid rows for catalog entries not yet in the grid.

### How to use the lineage map

`domains/data/grid/lineage-map.md` defines the pipeline zone sequence and the test IDs that validate each zone-to-zone transition. Use this to confirm that lineage is intact for a given pipeline by checking whether each transition's required test IDs have an implementation.

### How to use the maturity model

`domains/data/framework/maturity-model.md` describes four maturity levels (Reactive, Defined, Automated, Adversarial). Map an existing pipeline to a maturity level by checking the domain coverage matrix against the DPPF gap assessment. The model also defines the specific test IDs needed to advance to each level.

### Key data structures to parse

| File | Primary structure | Key fields |
|---|---|---|
| `domains/data/grid/test-grid.md` | Table | Tier, Category, Test Type, DAMA Dimension, On Fail, DPPF IDs |
| `domains/data/tests/<domain>.md` | DPPF catalog table | ID, Name, What it verifies, Defends against, Standards, Lifecycle |
| `domains/data/framework/dppf.md` | Checklist table per domain | ID, Name, Status, Notes |
| `domains/data/grid/lineage-map.md` | Validation table | Transition, Test IDs, Supporting IDs |
| `domains/data/tests/cross-validation-suite.md` | Sensibility catalog | SEN-001 to SEN-010 with tier and evaluation guidance |
